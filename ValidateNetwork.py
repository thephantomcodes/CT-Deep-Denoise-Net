...
epoch = 3
os.mkdir(f"validationdata/0/{epoch}/")
model = ctu.NeuralNetwork()
model.load_state_dict(torch.load(f"modelstates/0/model_{epoch}.pth"))
...
with torch.no_grad():
    for bat, (image, label, file_names) in enumerate(ct_valid_dataloader):
        ...
        for idx, fname in enumerate(file_names):
            ...
            scio.savemat(f"validationdata/0/{epoch}/" + fname_pred, {'imgPatch': np.array(imgPatch)})
